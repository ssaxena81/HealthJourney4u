...
export async function resetPassword(values: z.infer<typeof FinalResetPasswordSchema>): Promise<ForgotPasswordResult> {
  try {
    ...
    if (validatedValues.email ) {
        console.warn("Attempting password reset for unauthenticated user via custom code flow. This needs Firebase oobCode integration or a secure temporary token system.");
...
const DemographicsSchemaServer = z.object({
  ...
  isAgeCertified: z.boolean().optional(), // Made optional, will be true if age >= 18
